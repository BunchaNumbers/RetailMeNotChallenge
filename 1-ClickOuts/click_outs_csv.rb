...
def generate_CSV(fname, hash, mean, min, max, stddev)
	...
	CSV.open("#{fname}.csv", "w") do |csv|
		...
		csv << ["Time", "Clicks", "", "Min", "#{min[1]}", "#{min[0]}"]
		...
		hash.each do |time, clicks|
			...
			temp_arr = ["#{time}", "#{clicks}"]

			# 2nd, 3rd, and 4th line with the max, mean, and standard deviation data respectively
			temp_arr.concat([nil, "Max", "#{max[1]}","#{max[0]}"]) if counter == 1
			temp_arr.concat([nil, "Mean","#{mean}"]) if counter == 2
			temp_arr.concat([nil, "Std.dev","#{stddev}"]) if counter == 3
